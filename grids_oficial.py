...
file = ler_arquivo(nome_df="large_df.csv")
...
fig_tags = px.bar(df_tags, x="Qtde", y=df_tags.index, orientation="h",
                  color="Qtde", color_continuous_scale=px.colors.sequential.Magma)
fig_tags.update_layout(xaxis_title="Teste", title_text="Olá")
...
if pagina == "Dashboards":
    ...
    with st.expander("Vizualizar dashboards das métricas gerais"):
        ...
        pie.update_layout(title=dict(text="Teste", x=0.5,
        ...
        barras.update_layout(xaxis_tickangle=0, width=750, height=525,
    ...
    try:
        with col_direita:
            tabel = AgGrid(file, height=500, width=500, update_mode=update_mode_value, gridOptions=gd_options, theme="streamlit",
                           ...
                           data_return_mode=return_mode_value, fit_columns_on_grid_load=True, editable=False)
        # ----------grid atividade--------------
        ...
        with atividade_col:
            ...
            fig_atividade = px.bar(tabel_atividade.data, x=tabel_atividade.data.index, y="DevEUI", text_auto=True,
                                   template="plotly", title="ATIVIDADE DOS DEVEUI'S", color="DevEUI", color_continuous_scale=px.colors.sequential.OrRd,
                                   width=500, height=500)

            fig_atividade.update_layout(title_text="ATIVIDADE DOS DEVEUIS", xaxis_title=None, xaxis=dict(tickfont=dict(size=15)), yaxis=dict(tickfont=dict(size=15), titlefont=dict(size=18)),
                                        showlegend=True)
            ...
            fig_local = px.bar(tabel_status.data, x=tabel_status.data.index, y="DevEUI", text_auto=True,
                               template="plotly_dark", title="LOCAL DOS DEVEUI'S", color="DevEUI",
                               color_continuous_scale=px.colors.sequential.OrRd, labels=False,
                               width=500, height=500)

            fig_local.update_traces(
                textfont_size=14, textposition="outside", textangle=0)
            fig_local.update_layout(xaxis_title=None, xaxis=dict(tickfont=dict(
                size=15)), yaxis=dict(tickfont=dict(size=15), titlefont=dict(size=18)))
